...
<html lang="en" data-bs-theme="auto">
    <head>
        ...
        <link href="../title/css/headers.css" rel="stylesheet">
    </head>
    <body>
        <!-- header -->
        <main>
            <div class="container">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <ul class="nav">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="jb_profile.php">Profile</a>
                        </li>
                    </ul>
        ...
        <?php
            ...
            $query = "SELECT name FROM tbl_jobseekers WHERE id = $jbid";
            $result = mysqli_query($conn, $query);

            if ($result && mysqli_num_rows($result) > 0) {
                $jobseekerData = mysqli_fetch_assoc($result);
                $jobseekerName = $jobseekerData['name'];

                echo "<h3>Hi, $jobseekerName!</h3>";
            } else {
                echo "Error fetching jobseeker details: " . mysqli_error($conn);
            }
        ?>
        </div>
        <!-- SEARCH BAR --> 
                    ...
                    if ($resultJobs && mysqli_num_rows($resultJobs) > 0) {
                        while ($row = mysqli_fetch_assoc($resultJobs)) {
                            ...
                            <div class="col-md-4 crd" style="margin: 20px; background-color:#FFFFFF; padding: 20px;">
                                ...
                                <div class="d-flex justify-content-end mt-auto">
                                    <button class="btn btn-outline-primary ms-2 apply-btn" type="submit">Apply</button>
                                </div>
                            </div>
                            <?php
                        }
            ...
            <script>
                $(document).ready(function () {
                    $("#applyBtn").click(function () {
                        ...
                        var jbid = <?php echo $jbid; ?>;

                       var currentDate = new Date().toISOString().slice(0, 10);

                        $.ajax({
                            type: "POST",
                            url: "process_application.php?id=<?php echo $pid; ?>",
                            data: {
                                pid: pid,
                                jbid: jbid,
                                date: currentDate,
